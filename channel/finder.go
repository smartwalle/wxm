...
const (
	kGetFinderAttr = "https://api.weixin.qq.com/channels/finderlive/get_finder_attr_by_appid"
)
...
func (c *Channel) GetFinderAttr(ctx context.Context, accessToken string) (response *GetFinderAttrResponse, err error) {
	var request = struct{}{}
	if err = c.Post(ctx, accessToken, kGetFinderAttr, request, nil, &response); err != nil {
		return nil, err
	}
	return response, nil
}
