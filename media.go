...
func (m *MiniProgram) UploadTempMedia(mediaType MediaType, filename string) (result *UploadMediaResponse, err error) {
	if mediaType == "" {
	...
	if err = m.client.upload(http.MethodPost, kUploadMedia, "media", filename, values, &result); err != nil {
		return nil, err
	}
	return result, nil
}
